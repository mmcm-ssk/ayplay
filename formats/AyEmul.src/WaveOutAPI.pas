...
{ VisEventH:THANDLE;
 VisThreadID:DWORD;
 VisThreadH:THANDLE = 0;}
 WaveOutInitialized:boolean = False;

procedure WOVisualisation;
procedure StartWaveOut;
...
procedure SuspendIfWO;
procedure ResumeIfWO;

implementation

uses Common, Formats, AY, Errors, MainWin;
...
type
 EMultiMediaError = class(AYEmulError);
...
SetThreadPriority(VisThreadH,THREAD_PRIORITY_ABOVE_NORMAL)
end;}
...
repeat
 ...
 if not Real_End and (Reseted = 0) then
  begin
   for i := 0 to NumberOfBuffers - 1 do
...
begin
if WOThreadActive then exit;
if WaveOutInitialized then CloseWaveOut;
...
var
 ...
 i,bl:integer;
begin
if WaveOutInitialized then exit;
with pwfx.wf do
 begin
  wFormatTag := 1;
...
var
 i:integer;
begin
if not WaveOutInitialized then exit;
StopWOThread;

WaveOutInitialized := False;
...
procedure SuspendIfWO;
begin
if IsPlaying and WOThreadActive then
 WOResetPlaying(False)
end;

procedure ResumeIfWO;
begin
if IsPlaying and (Reseted > 0) and WOThreadActive then
 WOUnresetPlaying
end;
