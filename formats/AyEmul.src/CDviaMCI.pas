...
function CDIsStopped(CDNumber:DWORD):boolean;
...
uses KOL, Errors, Common, Formats, MainWin;
...
if Er <> 0 then
 begin
  s := 'MCI error #' + Int2Str(Er);
  if mciGetErrorString(Er,ErMes,256) then
   s := s + ': "' + ErMes + '"';
  raise AYEmulError.Create(s)
 end
end;
...
if (Length(CDDrives) = 0) then
 raise AYEmulError.Create('No CD drives found');
if DWORD(CDNumber) >= DWORD(Length(CDDrives)) then
 raise AYEmulError.Create('Specified CD drive not found')
end;
...
Result := MSP.dwReturn = MCI_MODE_STOP
end;
...
MPP.dwFrom := n;
MPP.dwTo := DWORD(n) + CDGetTrackLength(CDNumber,n) shl 8;
MCICheck(mciSendCommand(CDIDs[CDNumber],MCI_PLAY,
                             {$IFDEF WIN32GUI}
                             MCI_NOTIFY or
                             {$ENDIF WIN32GUI}
                             MCI_FROM or MCI_TO,DWORD(@MPP)));
CDPlayingPaused := False
end;
...
begin
 ...
 try
  CDPlayTrack(CDNumber,n,WHandle)
 except
