 ...
 begin
  if Size >= BufferSize then
   begin
    ...
    if Readen < Size then
     raise AYEmulError.Create(Mes_ReadAfterEndOfFile);
    inc(UniFilePos,Size)
   end
  else
   begin
...
with UniReadersData[Handle]^ do
 begin
  ...
  if Longword(Pos) > Longword(UniFileSize) then
   raise AYEmulError.Create(Mes_SeekAfterEndOfFile);
 ...
 begin
  ...
  URFile:
   begin
    ...
    Reset(UniFile,1);
    UniFileSize := System.FileSize(UniFile);
