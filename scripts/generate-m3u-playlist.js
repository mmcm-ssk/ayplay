...
const EXTENSIONS = ['.fym', '.pt3', '.vt2', '.psg', '.stc', '.ay', '.pt2', '.snd', '.asc', '.stp', '.pt1', '.psc', '.ftc', '.sqt', '.fxm'];
    ...
    for (var i = 0; i < items.length; i++) {
        ...
        if (stat.isDirectory()) {
            ...
            if (EXTENSIONS.includes(ext)) {
                ...
                var title = name.replace(/\.(fym|pt3|vt2|psg|stc|pt2|ay|snd|asc|stp|pt1|psc|ftc|sqt|fxm)$/i, '').replace(/%20/g, ' ');
                entries.push({
                    file: relative,
                    title: title,
