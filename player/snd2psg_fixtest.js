...
class SndToPsg {
	...
	parseSnd() {
			...
			for(var i=0; i<3; i++) {
				...
				this._nsFlag[regs[5]] = false;
				this._slideFlag[regs[5]] = false;
				...
				if(fileBytes[regs[5]+0o62] < 0) {
					...
					if(this._nsFlag[regs[5]] && this._prevNS[regs[5]] && !this._slideFlag[regs[5]] && !(fileBytes[regs[5]+0o62] < 0)
							&& regs[1] > 0 && regs[1] < this._prevO21[regs[5]] && regs[2] <= 4) regs[2]--;
					regs[1] -= regs[2];
					if(regs[1] >= 0) sel = 1;
				}
				if(sel == 0) regs[1] = 0;
				...
				this._prevNS[regs[5]] = !!this._nsFlag[regs[5]];
				this._prevO21[regs[5]] = fileBytes[regs[5]+0o21]&255;
	...
	p744() {
			...
			if ((regs[0]&255) === 1) {
				this._nsFlag[regs[5]] = true;
				this._slideFlag[regs[5]] = regs[3] < 0;
			}
