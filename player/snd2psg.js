...
// JavaScript class v1.4.2026
//
// Creates PSG dump from RockMonitor 3 SND module by Caroline software inc.
...
//		<script type="text/javascript" src="snd2psg.js?4"></script>
...
class SndToPsg {
	...
	vals = new Int16Array(new ArrayBuffer(6)); //vals 0,1,2
	endSong;
	envType;
	...
	readAndParseSND() {
			...
			resultPSG.push(0o375); //end of PSG dump
			return resultPSG;
	}
	...
	prepSnd() {
			...
			fileBytes[0o174] = fileBytes[regs[5]]&255;
			fileBytes[0o175] = fileBytes[regs[5]+1]&255;
			regs[5] = regs[5] + 2;
			...
			fileBytes[0o244] = regs[0];
			fileBytes[0o245] = regs[0]>>8;

			fileBytes[0o234] = regs[0];
			fileBytes[0o235] = regs[0]>>8;
			...
			fileBytes[0o300] = regs[0];
			fileBytes[0o301] = regs[0]>>8;

			fileBytes[0o2174] = regs[5];
			fileBytes[0o2175] = regs[5]>>8;

			regs[5] = 0o2520;
			regs[4] = 0o226;

			this.setWord(5, 0o52, 4);

			for(var i=0; i<3; i++) {
				this.setWord(5, 0, 0, 0);
				this.setWord(5, 4, fileBytes[0o2174]&255, fileBytes[0o2175]&255);
				this.setWord(5, 0o54, 0, 0);
				this.setWord(5, 0o57, 0o20, 0);
				regs[5] = regs[5] + 0o70;
			}
	}
	...
	parseSnd() {
			...
			for(var i=0; i<3; i++) {
				...
				if(fileBytes[regs[5]+0o14] < 0) {
					...
					this.setWord(5, 0o16, 0, 0);

					if(this.p1042() == false) {
						this.setWord(5, 0o20, 0, 0);
					}
					else {
						...
						regs[1] = fileBytes[regs[0]+0o46]&255;
						regs[2] = regs[1];
						regs[1] = regs[1] & 0o17;
				...
				if(fileBytes[regs[5]+0o15] < 0) {
					...
					this.setWord(5, 0o24, 0, 0);

					if(this.p1042()) {

						this.p700();

						regs[1] = fileBytes[regs[0]+0o63]&255;
						regs[2] = regs[1];

						regs[1] = regs[1] & 0o37;
						regs[2] = regs[2] & (~regs[1]);

						vals[0] = (fileBytes[regs[3]+0o12]&255) + ((fileBytes[regs[3]+0o13]&255)*256);
						if((vals[0] & regs[4]) !== 0) regs[2] += 0o20;
				...
				if(fileBytes[regs[5]+0o44] < 0) {
					...
					this.setWord(5, 0o32, 0, 0);
					this.setWord(5, 0o34, 0, 0);

					if (this.p1042()) {

						regs[0] = regs[0] << 1;
						regs[3] = (fileBytes[regs[5]+4]&255) + ((fileBytes[regs[5]+5]&255)*256);
						regs[0] = regs[0] + regs[3];
						regs[2] = (fileBytes[regs[0]+0o14]&255) + ((fileBytes[regs[0]+0o15]&255)*256);
						regs[1] = regs[2];

						regs[0] = 0o2000;
						regs[1] = regs[1] & 0o1777; //bic #176000

						if((regs[2] & regs[0]) !== 0) regs[1] = regs[1] | 0o176000;
						...
						regs[1] = regs[4];
						this.swabBytes(2);
						regs[2] = regs[2] & (~0o177417);

						this.p724();

						fileBytes[regs[5]+0o44] = regs[2];

						this.setWord(5, 0o50, 1);

						if(fileBytes[regs[3]+3] >= 0) {
							this.setWord(5, 0o40, 1);
						}
						else {
							regs[4] = 0;
							regs[1] = regs[1] - ((fileBytes[regs[5]+0o40]&255) + ((fileBytes[regs[5]+0o41]&255)*256));
							if(regs[1] < 0) {
								regs[1] = -regs[1];
								regs[4] = regs[4] + 1;
							}
							...
							fileBytes[regs[5]+0o34] = regs[1];
							fileBytes[regs[5]+0o35] = regs[1]>>8;

							regs[1] = 0;
							for(regs[0] = 0o10; regs[0] > 0; regs[0] = regs[0] - 1) {
								regs[1] = regs[1] << 1;
								regs[3] = regs[3] << 1;
								if(regs[3] >= regs[2]) {
									regs[3] -= regs[2];
				...
				vals[0] = (fileBytes[regs[0]]&255) + ((fileBytes[regs[0]+1]&255)*256);
				regs[0] += 2;
				vals[1] = (fileBytes[regs[0]]&255) + ((fileBytes[regs[0]+1]&255)*256);
				vals[1] += vals[0];

				this.setWord(0,0,0,0,1);

				if(fileBytes[regs[5]+0o14] == 0) {
					this.setWord(0, 0, fileBytes[regs[0]+2]&255, fileBytes[regs[0]+3]&255);
				}
				...
				if(sel == 0) regs[1] = 0;
				if(sel < 2) fileBytes[regs[4]] = regs[1];
				...
				if(((fileBytes[0o2777]&255) & (fileBytes[regs[5]+0o64]&255)) == 0) fileBytes[0o2776] = fileBytes[regs[5]+0o27];
				
				vals[0] = (fileBytes[regs[5]+0o32]&255) + ((fileBytes[regs[5]+0o33]&255)*256);
				vals[1] = (fileBytes[regs[2]]&255) + ((fileBytes[regs[2]+1]&255)*256);
				var c = ((vals[0]&0o177777)+(vals[1]&0o177777)) > 0o177777 ? 1 : 0;
				vals[1] += vals[0];

				this.setWord(2,0,0,0,1);
				regs[2] += 2;

				vals[0] = (fileBytes[regs[5]+0o34]&255) + ((fileBytes[regs[5]+0o35]&255)*256);
				vals[1] = (fileBytes[regs[2]]&255) + ((fileBytes[regs[2]+1]&255)*256);
				vals[1] += c;
				vals[1] += vals[0];
				...
				if(fileBytes[regs[5]+0o44] == 0) {
					this.setWord(2, 0, fileBytes[regs[5]+0o50]&255, fileBytes[regs[5]+0o51]&255);
				}

				this.setWord(4, 0, fileBytes[regs[2]]&255, fileBytes[regs[2]+1]&255);

				this.setWord(5, 0o60, 0, 0);
	...
	p114() {
			...
			fileBytes[0o2777] |= (fileBytes[regs[5]+0o64]&255);
	}
	...
	p744() {
			...
			fileBytes[regs[5]+0o23] = regs[1];

			if(regs[3] >= 0) {
				fileBytes[regs[5]+0o21] = regs[1];
				return;
			}
	...
	p140() {
			...
			for(regs[0] = 0o20; regs[0] > 0; regs[0] = regs[0] - 1) {
				var c = ((regs[1]&0o177777)&0o100000) !== 0 ? 1 : 0;
				regs[1] = regs[1] << 1;
				regs[3] = regs[3] << 1;
				regs[3] += c;
				if(regs[3] >= regs[2]) {
	...
	p1042() {
			...
			if(fileBytes[regs[5]+0o42] == 0) {
				if(regs[0] > fileBytes[regs[4]+0o10]) {
	...
	p172() {
			...
			while(1==1) {
				...
				while(1==1) {
					...
					if((regs[1]&255) == 0o377) {
						...
						this.setWord(5, 0o52, 1);

						regs[1] += ((fileBytes[regs[0]]&255) + ((fileBytes[regs[0]+1]&255)*256));
						regs[0] += 2;

						this.setWord(5, 0o142, 1);

						regs[1] += ((fileBytes[regs[0]]&255) + ((fileBytes[regs[0]+1]&255)*256));

						this.setWord(5, 0o232, 1);
					...
					if((regs[1]&255) !== 0) {
						...
						if((regs[1]&255) < 0o140) {
							...
							if((regs[0]&255) < 0o160) {
								...
								fileBytes[regs[5]+0o13] = (fileBytes[regs[5]+0o13]&255) & 0o177;
								continue;
							}
							...
							if((regs[0]&255) < 0o200) {
								regs[2] = 0o17;
								regs[2] -= regs[1];
								fileBytes[regs[5]+0o57] = regs[2];
								continue;
							}

							if((regs[0]&255) < 0o220) {
								regs[1] = regs[1] << 1;
								fileBytes[regs[5]+0o66] = regs[1];
								fileBytes[regs[5]+0o60] = regs[1];
								continue;
							}
							...
							if((regs[0]&255) < 0o240) {
								...
								regs[4] += 1;
								regs[2] = 0o362;
								fileBytes[regs[5]+0o62] = regs[2];
								continue;
							}

							regs[0] = regs[0] - 0o240;
							fileBytes[regs[5]+0o46] = regs[0];
							continue;
						}
			...
			if((fileBytes[regs[5]+0o42]&255) == 0) {
	...
	setWord(reg, offset, regS, hByte, val) {
		var regs = this.regs;
		var vals = this.vals;
		var fileBytes = this.fileBytes;

		offset -= (((regs[reg]&255)+offset)&1) == 0 ? 0 : 1;

		if(val !== undefined) {
			fileBytes[regs[reg]+offset] = vals[val];
			fileBytes[regs[reg]+offset+1] = vals[val]>>8;
		}
		else {
			if(hByte == undefined) {
					fileBytes[regs[reg]+offset] = regs[regS];
					fileBytes[regs[reg]+offset+1] = regs[regS]>>8;
			}
			else {
				fileBytes[regs[reg]+offset] = regS;
				fileBytes[regs[reg]+offset+1] = hByte;
			}
		}
	}
	swabBytes(reg) {
		var regs = this.regs;
		var byte1 = regs[reg]&255;
		var byte2 = (regs[reg]>>8)&255;
		regs[reg] = byte2 + (byte1*256);
	}
}
