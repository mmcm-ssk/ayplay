...
- **Waveform-визуализация** — осциллограмма/волновая форма текущего трека, выбор позиции по клику (с поддержкой предрассчитанных серверных JSON в `waveforms/` и фоновым воркером-fallback).
...
├── waveform_gen.bat          — скрипт предрасчёта волновых форм (waveform JSON) для всех треков
├── waveforms\                — предрассчитанные JSON волновых форм (зеркалирует структуру chiptunes/)
├── source\                   — архивные материалы, инструменты и старые документы (formats, tools, optimizm)
│
...
### Сборка/генерация
```bash
# сгенерировать предрассчитанные waveform JSON для всех треков (поддерживает все форматы и multi-subsong .ay)
